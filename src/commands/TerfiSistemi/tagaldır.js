...
let collector = await msg.createMessageComponentCollector({ filter, time: 60000 })
    ...
    collector.on("collect", async (button) => {
      ...
      client.channels.cache.find(x => x.name == "taglı_log").send({ embeds: [embed.setDescription(`
      Adlı Yetkili ${message.author} Adlı Kullanıcıyı ${member.toString()} Taglı Olarak Belirledi.
`)] });
