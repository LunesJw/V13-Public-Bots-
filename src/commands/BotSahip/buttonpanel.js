...
${client.emojis.cache.find(x => x.name === "mavinokta")} \`(${total} gerçek, ${bonus} bonus, ${leave} ayrılmış, ${fake} fake)\`
      
${client.emojis.cache.find(x => x.name === "mavinokta")} \`Günlük: ${daily}, Haftalık: ${weekly}, Taglı: ${tagged}\`
`, ephemeral: true });
}
...
{
await interaction.reply({ content: `
${client.emojis.cache.find(x => x.name === "mavinokta")} Sesli kanallardaki üye sayısı : \`${(interaction.guild.members.cache.filter((x) => x.voice.channel).size)}\`
${client.emojis.cache.find(x => x.name === "mavinokta")} Sunucudaki toplam üye sayısı : \`${(interaction.guild.memberCount)}\`
${client.emojis.cache.find(x => x.name === "mavinokta")} Sunucunun oluşturulma tarihi: \`${moment(interaction.guild.createdAt).locale("tr").format("LLL")}\`
${client.emojis.cache.find(x => x.name === "mavinokta")} Sunucu destek numarası : \`${(interaction.guild.id)}\`
`, ephemeral: true });
}
...
${client.emojis.cache.find(x => x.name === "mavinokta")} Toplam: \`${moment.duration(voiceData ? voiceData.topStat : 0).format("H [saat], m [dakika] s [saniye]")}\`

${client.emojis.cache.find(x => x.name === "mavinokta")} Haftalık Ses: \`${voiceWeekly}\`
${client.emojis.cache.find(x => x.name === "mavinokta")} Günlük Ses: \`${voiceDaily}\`
`, ephemeral: true });
}
