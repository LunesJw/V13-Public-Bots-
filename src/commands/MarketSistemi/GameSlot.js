const ramal = require("../../schemas/dolar");
let limit = new Map();
...
run: async (client, message, args, embed, prefix) => {
    ...
    let dolarData = await ramal.findOne({ guildID: message.guild.id, userID: message.author.id });  
    ...
    let ramalslot1 = slot[Math.floor(Math.random() * slot.length)];
    let ramalslot2 = slot[Math.floor(Math.random() * slot.length)];
    let ramalslot3 = slot[Math.floor(Math.random() * slot.length)];
...
setTimeout(() => {
if(ramalslot1 === ramalslot2 && ramalslot1 === ramalslot3 ) {
...
slotMessage.edit(`
\`___SLOTS___\`
  ${ramalslot1} ${ramalslot2} ${ramalslot3}
\`|         |\`
\`|         |\`
