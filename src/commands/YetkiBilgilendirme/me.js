...
embed.addField(`${star} **Mesaj İstatistiği**`, `
  ...
  ${client.emojis.cache.find(x => x.name === "mavinokta")} Haftalık Mesaj: \`${Number(messageWeekly).toLocaleString()} mesaj\`
  ${client.emojis.cache.find(x => x.name === "mavinokta")} Günlük Mesaj: \`${Number(messageDaily).toLocaleString()} mesaj\`
   `, false)
    ...
    collector.on("collect", async (button) => {
      ...
      \`• Son aktifliği     :\` ${seens.lastSeenVoice ? `<t:${String(seens.lastSeenVoice ? seens.lastSeenVoice : Date.now()).slice(0, 10)}:R>` : "Son Seste Görülme Bulunamadı!"}
      \`• İlerleme Durumu    :\` ${progressBar(coinData ? coinData.coin : 0, maxValue.coin, 9)} \`${coinData ? coinData.coin : 0} / ${maxValue.coin}\`
      ...
      ${client.emojis.cache.find(x => x.name === "unlem")} **Yetki Durumu;**
       ...
       \`• Yetki İlerleme Durumu: \` ${progressBar(coinData ? coinData.coin : 0, maxValue.coin, 9)} \`${coinData ? coinData.coin : 0} / ${maxValue.coin}\`
...
${client.emojis.cache.find(x => x.name === "mavinokta")} Kayıt: (\`Puan Etkisi: +${toplamData ? toplamData.toplams.length*5.5 : 0}\`)
${client.emojis.cache.find(x => x.name === "mavinokta")} Taglı: (\`Puan Etkisi: +${taggedData ? taggedData.taggeds.length*25 : 0}\`)
...
${client.emojis.cache.find(x => x.name === "mavinokta")} Toplam Aldığın Cezalar : ${cezapuanData ? cezapuanData.cezapuan.length : 0} (\`Toplam ${cezaData ? cezaData.ceza.length : 0}\`)
 `, false)

.addField(`${star} **Net Puanlama Bilgisi**`, `
${client.emojis.cache.find(x => x.name === "mavinokta")} Kayıt işlemi yaparak, \`+5.5\` puan kazanırsın.
${client.emojis.cache.find(x => x.name === "mavinokta")} Taglı üye belirleyerek, \`+25\` puan kazanırsınız.
${client.emojis.cache.find(x => x.name === "mavinokta")} İnsanları davet ederek, \`+15\` puan kazanırsın.
${client.emojis.cache.find(x => x.name === "mavinokta")} İnsanları yetkili yaparak, \`+30\` puan kazanırsın.
${client.emojis.cache.find(x => x.name === "mavinokta")} Seste kalarak, ortalama olarak \`+4\` puan kazanırsınız.
${client.emojis.cache.find(x => x.name === "mavinokta")} Yazı yazarak, ortalama olarak, \`+2\` puan kazanırsınız.
 `, false)
