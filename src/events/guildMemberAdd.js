...
\`\`\`json\n"Kayıt Olduktan Sonra Kurallar Kanalındaki Kurallarımızı Okumuş Kabul Edilirsiniz Ve Ceza-i İşlemler Bunu Bilerek Uygulanır"\n\`\`\``});
channel.wsend({ content:`${client.emojis.cache.find(x => x.name === "join")} ${member} Kullanıcısı \`Sunucu Özel URL\` tarafından davet edildi.`})
return }
...
channel.wsend({ content:`${client.emojis.cache.find(x => x.name === "join")} ${member} Kullanıcısı \`${usedInvite.inviter.tag}\` tarafından davet edildi.`})
